...
public class yonetici_eleman extends AppCompatActivity {
    EditText editText;
    ...
    public void yonetici_arama(View view){
        ...
        if(abone_no.length() ==0){
            ...
            try {
                ...
                c = db.rawQuery(" SELECT * FROM ELEMAN WHERE Eleman_no=" + abone_no, null);
                while (c.moveToNext()) {
                    if (c.getInt(0) != 0) {
    ...
    public void yonetici_sil(View vi){
        ...
        if(abone_no.length() ==0){
            ...
            try {
                ...
                c = db.rawQuery(" SELECT * FROM ELEMAN WHERE Eleman_No=" + abone_no, null);
                while (c.moveToNext()) {
                    db.delete("ELEMAN", "Eleman_No="+abone_no ,null);
                    db.close();
                }
    ...
    private void initialize() {
        ...
        listViewAdapter = new CustomListViewAdapter(yonetici_eleman.this,persons);
    ...
    private void fillArrayList(ArrayList<Person>persons){
        ...
        while(c.moveToNext()){
            if (c.getInt(0) != 0){
                Person person = new Person("ELEMAN NO:" + c.getString(0)+" DEPARTMAN:"+c.getString(7),
                        "ADI:"+c.getString(1)+" SOYADI:"+c.getString(2)
                        , R.drawable.employee,"MAAS:"+c.getString(8)+" SUBE NO:"+c.getString(5));
                persons.add(person);
                a++;
            }
        }
