...
public class yonetici_sube extends AppCompatActivity {
    EditText editText;
    ...
    public void yonetici_arama(View view){
        ...
        if(abone_no.length() ==0){
            ...
            try {
                ...
                c = db.rawQuery(" SELECT * FROM YUKLEME WHERE Sube_No=" + abone_no, null);
                while (c.moveToNext()) {
                    if (c.getInt(0) != 0) {
                        Person person = new Person("Sube No:" + c.getString(0),
                                "Adres No:"+c.getString(1)+" Sube Türü:"+c.getString(2)
                                , R.drawable.company,"Kira:"+c.getString(3)+" Calisan Sayisi:"+c.getString(4));
                        persons.add(person);
                        a++;
                    }
                }
    ...
    public void yonetici_sil(View vi){
        ...
        if(abone_no.length() ==0){
            ...
            try {
                ...
                c = db.rawQuery(" SELECT * FROM YUKLEME WHERE Sube_No=" + abone_no, null);
                while (c.moveToNext()) {
                    db.delete("YUKLEME", "Sube_No="+abone_no ,null);
                    db.close();
                }
    ...
    private void initialize() {
        ...
        listViewAdapter = new CustomListViewAdapter(yonetici_sube.this,persons);
    ...
    private void fillArrayList(ArrayList<Person>persons){
        ...
        try{
            ...
            dbHelper.createDatabase("DATABASE.db");
            db = dbHelper.getReadableDatabase();
            c = db.rawQuery(" SELECT * FROM YUKLEME",null);
        }catch(Exception e){
            Log.e("DB_LOG",e.getMessage());
            Log.e("DB_LOG","Veritabanı oluşturulamadı veya kopyalanamadı !");
        ...
        while(c.moveToNext()){
            if (c.getInt(0) != 0){
                Person person = new Person("Sube No:" + c.getString(0),
                        "Adres No:"+c.getString(1)+" Sube Türü:"+c.getString(2)
                        , R.drawable.users,"Kira:"+c.getString(3)+" Calisan Sayisi:"+c.getString(4));
                persons.add(person);
                a++;
            }
        }
